...
lambda_values = np.logspace(-4, 4, 50)  # Creates 50 evenly spaced values on log scale from 10^-4 to 10^4
...
for lambda_val in lambda_values:
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X), 1):
        ...
        # Train Lasso model with current lambda
...
plt.legend()
plt.tight_layout()
plt.savefig('lasso_lambda_vs_rmse.png')
plt.close() 